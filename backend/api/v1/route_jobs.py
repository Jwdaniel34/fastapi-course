...
@router.post("/create-job", response_model= ShowJob) #/job/create-job
...
@router.get("/get/{id}", response_model= ShowJob) #/job/get/{id}
...
@router.get("/all", response_model=List[ShowJob]) #/job/all
def retrieve_list_jobs(db: Session= Depends(get_db)):
    job = list_jobs(db=db)
...
def update_job(id: int, job: JobCreate, db: Session = Depends(get_db)):
    owner_id = 1
    message = update_job_by_id(id=id, job= job, owner_id= owner_id, db= db)
    if not message:
        raise HTTPException(status_code= status.HTTP_404_NOT_FOUND,
                                detail= f"Job with id {id} does not exist")
